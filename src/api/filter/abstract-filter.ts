...
export abstract class AbstractFilter<T extends AbstractModel> {
  ...
  public async startDatabaseQuery(...context: StateContext[]): Promise<CollectionModelDatabaseResult<T>> {
    this.databaseResult = await this.executeDatabaseQuery(context)
    return this.databaseResult
  }

  protected abstract executeDatabaseQuery(context: StateContext[]): Promise<CollectionModelDatabaseResult<T>>
